    ...
    <div className="relative flex flex-col lg:grid lg:grid-cols-[340px_1fr] h-full w-full min-h-0 overflow-hidden bg-hw-bg">
      ...
      `}>
        ...
        <div className="flex lg:hidden justify-between items-center pb-2 border-b border-hw-border">
          <span className="text-[10px] tracking-wider text-hw-cyan font-bold font-mono">// AJUSTES</span>
          <button
            onClick={() => setShowMobileControls(false)}
        ...
        <div className="absolute top-4 right-4 w-[245px] sm:w-[260px] max-w-[calc(100%-2rem)] bg-hw-panel/95 backdrop-blur-md border border-hw-border rounded shadow-lg p-3 sm:p-4 z-10 select-none transition-all duration-300">
          ...
          {showHUD ? (
            <div className="animate-in fade-in duration-200 mt-2">
              ...
              <div className="text-[11px] sm:text-xs text-gray-400 mt-1.5 sm:mt-2.5 leading-relaxed min-h-[32px] font-mono">
                {divergedAt !== null
              ...
              <div className="flex flex-col gap-1.5 mt-2.5 border-t border-hw-border pt-2.5 text-[10px] sm:text-[10.5px] font-mono uppercase">
                <span className="flex items-center gap-2 text-gray-400">
                  <span className="w-2.5 h-2.5 bg-hw-cyan animate-pulse" /> original (6 decs)
                </span>
                <span className="flex items-center gap-2 text-gray-400">
                  <span className="w-2.5 h-2.5 bg-hw-amber animate-pulse" /> re-corrido ({precision} decs)
                </span>
              </div>
            </div>
          ) : (
            <div className="flex items-baseline justify-between mt-1 text-xs font-mono animate-in fade-in duration-200">
              ...
              <span className={`font-bold ${divergedAt !== null ? "text-red-500" : "text-hw-cyan"}`}>
                {divergedAt !== null ? `${Math.round(divergedAt)} DÍAS` : "ESTABLE"}
              </span>
            </div>
          )}
